...
(defn multiples-archivos-paralelos [arch]
  (def nombres (leer-nombres arch))
  (def len (with-open [rdr (clojure.java.io/reader arch)] (count (line-seq rdr))))
  (cond
    (>= len 100) (doall (pmap (fn [l] (doall (map main l))) (partition-all (Math/ceil (/ 100 len)) (leer-nombres arch))))
    (< len 10) (doall (pmap (fn [l] (doall (map main l))) (partition-all 1 (leer-nombres arch))))
    true (doall (pmap (fn [l] (doall (map main l))) (partition-all (Math/ceil (/ 10 len)) (leer-nombres arch))))))
